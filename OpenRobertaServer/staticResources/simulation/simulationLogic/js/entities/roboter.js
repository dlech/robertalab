...
game.RoboterEntity = me.Entity.extend({
    ...
    update : function(dt) {
        ...
        if (!PROGRAM_SIMULATION.isTerminated()) {
            step();
            setSpeed(ACTORS.getLeftMotor().getPower(), ACTORS.getRightMotor().getPower());
            if (ACTORS.isResetTachoSensor()) {
                // resetWheelRotationCounter();
                ACTORS.setResetTachoSensor(false);
            }
            ACTORS.getLeftMotor().setCurrentRotations(getLeftDistance());
            ACTORS.getRightMotor().setCurrentRotations(getRightDistance());
